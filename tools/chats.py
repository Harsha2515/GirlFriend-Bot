...
viewer for it, so you never have to write SQL by hand.
    ...
    python tools/chats.py chat 123456789 -n 100 # last 100 messages
    ...
    python tools/chats.py reminders 123456789   # their pending nudges
...
def _user_row(conn, user_id):
    ...
    if not row:
        sys.exit(f"No user {user_id}. Run: python tools/chats.py users")
    return row


def cmd_chat(conn, args):
    user = _user_row(conn, args.user_id)
    tz = user["timezone"] or ""

    rows = conn.execute(
        "SELECT role, content, persona, created_at FROM messages "
        "WHERE user_id = ? ORDER BY id DESC LIMIT ?",
        (args.user_id, args.number),
    ).fetchall()
    rows = list(reversed(rows))

    if not rows:
        print(f"No messages stored for {user['first_name']} ({args.user_id}).")
        return

    print(f"Conversation with {user['first_name']} ({args.user_id}) — "
...
def cmd_facts(conn, args):
    ...
    if not rows:
        ...
        return
    print(f"What the bot remembers about {user['first_name']} ({args.user_id}):\n")
    for r in rows:
...
def cmd_reminders(conn, args):
    ...
    if not rows:
        ...
        return
    print(f"Reminders for {user['first_name']} ({args.user_id}), times in {tz or 'UTC'}:\n")
    for r in rows:
...
def cmd_export(conn, args):
    ...
    if not rows:
        sys.exit(f"No messages to export for {args.user_id}.")

    out = args.out or f"chat-{args.user_id}.txt"
    with open(out, "w", encoding="utf-8") as fh:
        fh.write(f"Conversation with {user['first_name']} ({args.user_id})\n")
